...
export function TaskRow({ task, onChange, onDelete }: Props) {
  ...
  return (
    <div className={`task-row ${task.completed ? "completed" : ""}`}>
      <button className={`complete-button ${task.completed ? "checked" : ""}`} aria-label={task.completed ? `Mark ${task.title} incomplete` : `Mark ${task.title} complete`} onClick={() => void onChange({ ...task, completed: !task.completed })}>
