 ...
 * external WebGL library. Adapted for Prior tiles: no `cn` dependency, a
 * `timeScale` prop to energize the motion (thinking) or calm it (idle), and
 * a guarded GL setup that leaves the canvas blank if WebGL is unavailable
 * so the CSS fallback behind it shows through.
 */
...
/**
 * Derive a box-free shader from a base source: the black background becomes
 * transparent while the wave keeps its original colors (luminance-kept).
 */
export function buildSiriShader(variant: SiriWaveVariant): string {
  return FRAGMENT_SHADERS[variant].replace(
    "fragColor = vec4(col, 1.0);",
    `float lum = dot(col, vec3(0.299, 0.587, 0.114));
    fragColor = vec4(col, clamp(lum * 2.0, 0.0, 1.0));`,
  );
}

export interface SiriWaveProps extends Omit<HTMLAttributes<HTMLCanvasElement>, "children"> {
  /** Which shader to render. */
  variant?: SiriWaveVariant;
  /** Canvas backing size in px (display size comes from CSS). */
  size?: number;
  /** Internal render resolution multiplier (lower = cheaper). */
  renderScale?: number;
  /** Time multiplier: >1 energizes the motion (thinking), <1 calms it (idle). */
  timeScale?: number;
}
  ...
  useEffect(() => {
    ...
    try {
      const compile = (type: number, src: string) => {
        ...
        if (!shader || !gl) throw new Error("shader creation failed");
      ...
      const fs = compile(gl.FRAGMENT_SHADER, buildSiriShader(variant));
      ...
      const dim = Math.max(2, Math.round(size * renderScale));
      canvas.width = dim;
      canvas.height = dim;
      ...
      // Leave the canvas blank so the CSS fallback behind it shows through.
      return;
    }
